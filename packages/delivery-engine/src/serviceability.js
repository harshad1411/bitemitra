...
export function resolveServiceability(point, { cities, zones, serviceAreas }) {
  ...
  if (outsideArea)
    return { serviceable: false, reason: 'OUTSIDE_SERVICE_AREA', ...outsideArea, serviceAreaId: null };
...
function inArea(point, area) {
  ...
  return (
    haversineM(point, { lat: Number(area.centerLat), lng: Number(area.centerLng) }) <= Number(area.radiusM)
  );
}
