...
describe('geo', () => {
  ...
  it('bbox', () => {
    expect(bboxOf(square(72.38, 23.79, 72.41, 23.82))).toEqual({
      minLat: 23.79,
      minLng: 72.38,
      maxLat: 23.82,
      maxLng: 72.41,
...
describe('serviceability', () => {
  ...
  const zones = [
    {
      ...
      slug: 'central',
      isActive: true,
      geometry: square(72.38, 23.79, 72.41, 23.82),
    },
    {
      id: 'z-east',
      cityId: 'unjha',
      ...
      slug: 'east',
      isActive: true,
      geometry: square(72.41, 23.79, 72.44, 23.82),
    },
    {
      id: 'z-closed',
      cityId: 'unjha',
      ...
      slug: 'closed',
      isActive: false,
      geometry: square(72.44, 23.79, 72.47, 23.82),
    },
    {
      id: 'z-meh',
      cityId: 'mehsana',
      ...
      slug: 'm',
      isActive: true,
      geometry: square(72.35, 23.57, 72.4, 23.62),
    },
  ];
  const serviceAreas = [
    {
  ...
  it('zone with service areas requires being inside one', () => {
    expect(resolveServiceability({ lat: 23.805, lng: 72.426 }, data)).toMatchObject({
      serviceable: true,
      serviceAreaId: 'sa-radius',
    });
    expect(resolveServiceability({ lat: 23.818, lng: 72.438 }, data)).toMatchObject({
      serviceable: false,
  ...
  it('inactive zones and non-live cities are not serviceable', () => {
    expect(resolveServiceability({ lat: 23.8, lng: 72.45 }, data)).toMatchObject({
      serviceable: false,
      reason: 'NO_CITY',
    });
    expect(resolveServiceability({ lat: 23.6, lng: 72.37 }, data)).toMatchObject({
      serviceable: false,
      reason: 'CITY_NOT_LIVE',
      city: { id: 'mehsana' },
    });
    expect(resolveServiceability({ lat: 10, lng: 10 }, data)).toMatchObject({
      serviceable: false,
      reason: 'NO_CITY',
    });
  });
