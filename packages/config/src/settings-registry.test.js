...
describe('settings registry', () => {
  ...
  it('money amounts are integers (no floats)', () => {
    expect(
      validateSetting('cod', 'CITY', { ...getSettingDefinition('cod').default, maxOrderValuePaise: 100.5 })
        .ok,
    ).toBe(false);
  });
  ...
  it('rejects unknown keys, disallowed scopes and invalid values', () => {
    ...
    expect(
      validateSetting('auth.otp', 'GLOBAL', {
        ttlSec: 5,
        maxAttempts: 5,
    ...
    expect(
      validateSetting('orders.limits', 'CITY', {
        minOrderPaise: 9900,
        maxOrderPaise: 5_000_000,
        maxItems: 50,
      }).ok,
    ).toBe(true);
  });
