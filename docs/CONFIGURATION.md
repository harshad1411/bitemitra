...
Status: **Phase 8 reads** `settlements.restaurants` (schedule per restaurant, weekly run day, minimum payout; the reserve is not applied yet), `settlements.riders` and `cod.netAgainstEarnings`. **Phase 7 reads** `payments.methods` (checkout, the quote's `paymentMethods`, the payment expiry) and `refunds.approval` (maker-checker). **Phase 6 adds** `riders.requiredDocuments`, `maps.provider` (Google road distance, D-81) and dispatch location/distance limits. **Phase 2 enables RESTAURANT and BRANCH overrides** (edited on the restaurant's Settings tab, D-40) and adds three `restaurants.*` keys. **Phase 1 implements** the settings registry, hierarchical resolution, settings API + history, feature flags, app version policies and remote config (`packages/config`, API `configuration` module, Admin → Settings). Covers MASTER_SPEC §2, §51, §63, §64, §65, §72, §73.
