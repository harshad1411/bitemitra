...
| Mobile foundations & screens | Jest-Expo + React Native Testing Library | gates, login flow, shells, error/loading states; customer: location → home → menu → customise → cart bill → coupon, sign-in when needed, denied location, unpublished pages; Phase 5: checkout → tracking with a realtime update, cancel, price changed, retry with the same idempotency key; restaurant: new-order alert starts/stops, accept, reject, ready, realtime arrival, conflicts | yes |
