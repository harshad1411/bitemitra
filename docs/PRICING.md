...
Status: **Phase 0 design.** Implemented in Phase 4 (`packages/pricing-engine`) with the test matrix below
written *before* the implementation. Items marked **⚠ Qn** depend on an owner decision in
[DECISIONS.md](DECISIONS.md#critical-questions); the stated default is used until answered.
...
`GLOBAL → COUNTRY → STATE → CITY → ZONE → RESTAURANT → CATEGORY → PRODUCT → VARIANT`
...
**⚠ Q7-b (precedence of platform-wide category rules):** following the spec literally, a platform-wide
...
### 2.3 Worked precedence example (spec §10)
...
| Commission | `{ type: PERCENTAGE\|FIXED\|HYBRID, rateBps?, fixedPaise?, hybridMode: SUM\|MAX, base: FOOD_VALUE_AFTER_RESTAURANT_DISCOUNT\|FOOD_VALUE }` |
| Tax | `{ appliesTo: FOOD\|PACKAGING\|DELIVERY_FEE\|PLATFORM_FEE\|SMALL_ORDER_FEE\|SURCHARGE\|COMMISSION, mode: INCLUSIVE\|EXCLUSIVE\|EXEMPT, components: [{ code, rateBps }], liableParty: PLATFORM\|RESTAURANT }` |
...
4. **Discounts.** Automatic promotion(s) first, then at most one coupon (stacking configurable, default: one promotion + one coupon). Eligibility (min order, first order, targeting, per-user/usage limits, payment method) evaluated on the **pre-discount food subtotal**. Discount computed on the display food subtotal **⚠ Q7-a**, capped by `maxDiscountPaise`, never > subtotal. Funding split: PLATFORM / RESTAURANT / SHARED(`restaurantShareBps`). The discount is allocated across lines (largest remainder) so tax can be computed per line.
5. **Packaging** (restaurant-configured, per item or per order, passed through to the restaurant, no markup).
6. **Food tax** per line on `(line display total − allocated discount)`, using the line's resolved tax rule (inclusive: extracted, exclusive: added, exempt: 0). Packaging taxed by its own rule (default: same as food). **⚠ Q3**
...
11. **Charge taxes** — delivery fee, surcharges, small-order fee, platform fee each use their own tax rule. **⚠ Q3**
12. **Tip** (if the `tips` flag is on) — not taxed, not commissioned, 100% to rider. **⚠ Q10**
13. **Final rounding** of the payable (setting `pricing.finalRounding`, default nearest ₹1, half-up). The difference is a visible `roundingAdjustment` line absorbed by the platform. **⚠ Q10**
...
- **Commission** = rule applied to the commission base (default: food value − restaurant-funded discount **⚠ Q7-a**). HYBRID = fixed + rate (SUM) or max of the two (MAX).
- **Tax on commission** per the COMMISSION tax rule (default 18% GST — **⚠ Q3**).
- **Withholdings** (e.g. GST TCS / income-tax TDS on e-commerce payouts): modelled as configurable deduction rules, **default 0 until confirmed — ⚠ Q3**.
...
## 6. Worked example (illustrative numbers; tax treatment pending ⚠ Q3)
...
engine version; all rule ids consulted. The customer app receives only the customer-facing subset.
...
## 11. Open tax / legal questions (owner + CA must decide — ⚠ Q3, Q4)

Defaults exist only so development can proceed; **no production launch until confirmed**:
