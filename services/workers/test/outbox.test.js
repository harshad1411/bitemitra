...
describe('outbox relay', () => {
  it('runs handlers and marks events published', async () => {
    ...
    const relay = createOutboxRelay({
      ...
      clock,
      workerId: 'w1',
      handlers: { 'test.event': async (e) => seen.push(e.id) },
    });
    const a = await event();
    const b = await event();
  ...
  it('two workers never process the same event (FOR UPDATE SKIP LOCKED)', async () => {
    ...
    const w1 = createOutboxRelay({
      prisma,
      log,
      clock,
      workerId: 'w1',
      batchSize: 5,
      handlers: { 'test.event': slow },
    });
    const w2 = createOutboxRelay({
      prisma,
      log,
      clock,
      workerId: 'w2',
...
describe('media.uploaded handler', () => {
  ...
  it('generates WebP renditions and marks the media READY', async () => {
    const jpg = await sharp({ create: { width: 1600, height: 900, channels: 3, background: '#5B2A86' } })
      .jpeg()
      .toBuffer();
  ...
  it('small images are not enlarged; unreadable images are marked FAILED and parked', async () => {
    const png = await sharp({ create: { width: 100, height: 50, channels: 3, background: '#fff' } })
      .png()
      .toBuffer();
    ...
    const relay = createOutboxRelay({
      ...
      workerId: 'w',
      maxAttempts: 1,
      handlers: { 'media.uploaded': handler },
    });
