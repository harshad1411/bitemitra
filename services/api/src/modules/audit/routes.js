...
export default async function auditRoutes(app) {
  app.get('/v1/admin/audit-logs', { config: { permission: 'audit.view' } }, async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
    ...
    const actors = await actorsById(app.prisma, rows.map((r) => r.actorId));
    return toPage(rows.map((r) => ({ ...r, actor: actors.get(r.actorId) ?? null })), q.limit);
  });
}
