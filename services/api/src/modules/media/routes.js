...
export default async function mediaRoutes(app) {
  ...
  app.post(
    '/v1/admin/media',
    { config: { permission: 'media.manage' } },
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request, reply) => {
      const part = await request
        .file({ limits: { fileSize: env.MEDIA_MAX_BYTES, files: 1, fields: 10 } })
        .catch((err) => {
          throw new AppError('VALIDATION_FAILED', 'Upload a single image as multipart/form-data.', {
            details: { cause: err.code },
          });
        });
      if (!part)
        throw new AppError('VALIDATION_FAILED', 'No file uploaded.', { fieldErrors: { file: ['Required'] } });
      let buffer;
      try {
        ...
        if (/** @type {any} */ (err).code === 'FST_REQ_FILE_TOO_LARGE') {
          throw new AppError(
            'PAYLOAD_TOO_LARGE',
            `Images must be at most ${Math.floor(env.MEDIA_MAX_BYTES / 1024 / 1024)} MB.`,
          );
        }
        throw err;
      }
      ...
      if (!type)
        throw new AppError('UNSUPPORTED_MEDIA_TYPE', 'Only JPEG, PNG and WebP images are supported.');
      const field = (name) => {
        ...
        const media = await prisma
          .$transaction(async (tx) => {
            ...
            await enqueueEvent(tx, {
              ...
              payload: { mediaId: m.id },
            });
            await audit(tx, request, {
              action: 'media.upload',
              entityType: 'media',
              entityId: m.id,
              newValue: { title: m.title, mimeType: m.mimeType, sizeBytes: m.sizeBytes },
            });
            return m;
          })
          .catch(async (err) => {
  ...
  app.get(
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      const q = parse(
        pageQuery.extend({ status: z.enum(['PENDING', 'READY', 'FAILED']).optional() }),
        request.query,
      );
      const page = idPage(q);
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      ...
      return prisma.$transaction(async (tx) => {
        ...
        await audit(tx, request, {
          ...
          entityType: 'media',
          entityId: id,
          oldValue: { altText: before.altText, title: before.title, tags: before.tags },
          newValue: body,
        });
        return dto(after);
      });
  ...
  app.get(
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request, reply) => {
      ...
      const m =
        /^media\/([0-9a-f-]{36})\/(original\.(?:jpg|png|webp)|thumb\.webp|small\.webp|medium\.webp)$/.exec(
          key,
        );
