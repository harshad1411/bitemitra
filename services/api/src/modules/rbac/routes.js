...
export default async function rbacRoutes(app) {
  ...
  function assertNoEscalation(request, permissionKeys) {
    ...
    if (missing.length)
      throw forbidden(`You cannot grant permissions you do not have: ${missing.join(', ')}.`);
  }

  async function loadGrantRoles(grants) {
    const roles = await prisma.role.findMany({
      where: { id: { in: grants.map((g) => g.roleId) } },
    ...
    for (const g of grants) {
      ...
      if (!role)
        throw new AppError('VALIDATION_FAILED', 'Unknown role.', {
          fieldErrors: { grants: [`Unknown role ${g.roleId}`] },
        });
      if (CITY_SCOPED_ROLES.has(role.key) && !g.cityId)
        throw new AppError('VALIDATION_FAILED', `${role.name} must be limited to a city.`, {
          fieldErrors: { grants: ['City required'] },
        });
    }
    const cityIds = grants.map((g) => g.cityId).filter(Boolean);
    if (
      cityIds.length &&
      (await prisma.city.count({ where: { id: { in: cityIds } } })) !== new Set(cityIds).size
    ) {
      throw new AppError('VALIDATION_FAILED', 'Unknown city in grants.', {
        fieldErrors: { grants: ['Unknown city'] },
      });
    }
    return byId;
  }
  ...
  function assertGrantsAllowed(request, grants, byId) {
    for (const g of grants) {
      const role = byId.get(g.roleId);
      if (role.key === 'SUPER_ADMIN' && !heldBy(request).has('rbac.super'))
        throw forbidden('Only a Super Admin can grant Super Admin.');
      assertNoEscalation(
        request,
  ...
  const activeSuperAdmins = (tx, excludingAdminId) =>
    tx.adminUser.count({
      where: {
        isActive: true,
        id: excludingAdminId ? { not: excludingAdminId } : undefined,
        roles: { some: { role: { key: 'SUPER_ADMIN' }, cityId: null } },
      },
    });

  // ── Permissions & roles ──
  app.get('/v1/admin/permissions', { config: { permission: 'roles.view' } }, async () => ({
    items: PERMISSIONS,
  }));
  ...
  app.post(
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request, reply) => {
      ...
      if (unknown.length)
        throw new AppError('VALIDATION_FAILED', 'Unknown permissions.', {
          fieldErrors: { permissions: unknown },
        });
      if (body.permissions.includes('rbac.super'))
        throw forbidden('rbac.super is reserved for the Super Admin role.');
  ...
  app.patch(
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      const { id } = parse(idParam, request.params);
      const body = parse(
        roleUpdateBody.extend({ reason: z.string().trim().min(3).max(500).optional() }),
        request.body,
      );
      return prisma.$transaction(async (tx) => {
        ...
        if (before.key === 'SUPER_ADMIN')
          throw forbidden('The Super Admin role always has every permission and cannot be edited.');
        if (body.permissions) {
          const unknown = body.permissions.filter((p) => !isPermission(p));
          if (unknown.length)
            throw new AppError('VALIDATION_FAILED', 'Unknown permissions.', {
          ...
          if (!body.reason)
            throw new AppError('VALIDATION_FAILED', 'A reason is required when changing permissions.', {
        ...
        await audit(tx, request, {
          ...
          entityType: 'role',
          entityId: id,
          oldValue: roleDto(before),
          newValue: { ...roleDto(after), reason: body.reason ?? null },
        });
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request, reply) => {
      ...
      await prisma.$transaction(async (tx) => {
        const role = await tx.role.findUnique({
          where: { id },
          include: { _count: { select: { admins: true } }, permissions: { include: { permission: true } } },
        });
        if (!role) throw notFound('Role');
  ...
  app.post(
    '/v1/admin/users',
    { config: { permission: 'admins.manage' } },
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request, reply) => {
      ...
      return withIdempotency(
        request,
        reply,
        { successStatus: 201, fingerprint: { ...body, password: undefined } },
        () =>
          prisma.$transaction(async (tx) => {
            ...
            const user = await tx.user.create({
              data: {
                ...
                adminUser: {
                  create: {
                    roles: {
                      create: body.grants.map((g) => ({ roleId: g.roleId, cityId: g.cityId ?? null })),
                    },
                  },
                },
              },
  ...
  app.patch(
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (targetIsSuper && !heldBy(request).has('rbac.super'))
          throw forbidden('Only a Super Admin can change a Super Admin.');
        ...
        if ((body.isActive !== undefined || body.grants) && !body.reason) {
          throw new AppError('VALIDATION_FAILED', 'A reason is required to change status or roles.', {
            fieldErrors: { reason: ['Required'] },
          });
        }
        const losesSuper =
          targetIsSuper &&
          (body.isActive === false ||
            (body.grants && !body.grants.some((g) => byId.get(g.roleId).key === 'SUPER_ADMIN' && !g.cityId)));
        if (losesSuper && (await activeSuperAdmins(tx, id)) === 0)
          throw conflict('This is the last active Super Admin.');

        if (body.grants) {
          await tx.adminUserRole.deleteMany({ where: { adminUserId: id } });
          await tx.adminUserRole.createMany({
            data: body.grants.map((g) => ({ adminUserId: id, roleId: g.roleId, cityId: g.cityId ?? null })),
          });
        }
        ...
        const after = await tx.adminUser.findUniqueOrThrow({
          where: { id },
          include: { user: true, roles: { include: { role: true } } },
        });
        await audit(tx, request, {
          ...
          entityType: 'admin_user',
          entityId: id,
          oldValue: adminDto(before),
          newValue: { ...adminDto(after), reason: body.reason ?? null },
        });
        return { before, after };
      });
