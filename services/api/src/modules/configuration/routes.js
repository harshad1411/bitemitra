...
export default async function configurationRoutes(app) {
  ...
  app.get(
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request, reply) => {
      ...
      return config.remoteConfig(request.client, {
  ...
  async function assertCanConfigure(request, scope, scopeRefId, permission) {
    ...
    if (!ctx.cityId || !canInCity(request, permission, ctx.cityId))
      throw forbidden('You can only configure your own city.');
  }

  app.get(
    '/v1/admin/settings',
    { config: { permission: 'config.view' } },
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      const q = parse(
        z.object({ scope: configScope.default('GLOBAL'), scopeRefId: uuid.optional() }),
        request.query,
      );
      await assertCanConfigure(request, q.scope, q.scopeRefId ?? null, 'config.view');
      return {
        scope: q.scope,
        scopeRefId: q.scopeRefId ?? null,
        items: await config.listForScope(q.scope, q.scopeRefId ?? null),
      };
    },
  );
  ...
  app.put(
    '/v1/admin/settings',
    { config: { permission: 'config.manage' } },
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
  ...
  app.get(
    ...
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      ...
      const rows = await prisma.settingHistory.findMany({
        where: { ...(q.key ? { key: q.key } : {}), ...page.where },
      ...
      return toPage(
        rows.map((r) => ({ ...r, changedBy: actors.get(r.changedById) ?? null })),
        q.limit,
      );
    },
  );
  ...
  // ── Admin: feature flags ──
  app.get('/v1/admin/flags', { config: { permission: 'config.view' } }, async () => ({
    items: await prisma.featureFlag.findMany({ orderBy: { key: 'asc' } }),
  }));

  app.put(
    '/v1/admin/flags/:key',
    { config: { permission: 'flags.manage' } },
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      ...
      const updated = await prisma.$transaction(async (tx) => {
        ...
        await audit(tx, request, {
          ...
          newValue: { ...pick(after), reason: body.reason ?? null },
        });
        return after;
      });
      config.invalidate();
  ...
  app.put(
    '/v1/admin/app-versions/:appId/:platform',
    { config: { permission: 'config.manage' } },
    async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
      const params = parse(
        z.object({
          appId: z.enum(['CUSTOMER', 'RESTAURANT', 'RIDER']),
      ...
      if (compareVersions(body.minSupportedVersion, body.recommendedVersion) > 0) {
        throw new AppError(
          'VALIDATION_FAILED',
          'Minimum version cannot be higher than the recommended version.',
          { fieldErrors: { minSupportedVersion: ['Must be ≤ recommended version'] } },
        );
      }
      ...
      const saved = await prisma.$transaction(async (tx) => {
        ...
        if (
          before &&
          compareVersions(body.minSupportedVersion, before.minSupportedVersion) > 0 &&
          !body.reason
        ) {
          throw new AppError(
            'VALIDATION_FAILED',
            'A reason is required when raising the minimum version (it forces users to update).',
        ...
        const after = await tx.appVersionPolicy.upsert({
          where,
          create: { ...params, ...data },
          update: data,
        });
        await audit(tx, request, {
          action: 'app_version_policy.update',
          entityType: 'app_version_policy',
          entityId: `${params.appId}:${params.platform}`,
          oldValue: before,
          newValue: { ...after, reason: body.reason ?? null },
        });
        return after;
      });
      config.invalidate();
