...
export default async function dashboardRoutes(app) {
  app.get('/v1/admin/dashboard', { config: { permission: 'dashboard.view' } }, async (/** @type {import('../../core/types.js').JamzoRequest} */ request) => {
    const p = app.prisma;
    const cities = allowedCityIds(request, 'dashboard.view');
    const cityFilter = cities ? { id: { in: cities } } : {};
    const [citiesTotal, citiesLive, zones, admins, restaurantsByStatus, ridersByStatus, customers, media, recentAudit] = await Promise.all([
      ...
      p.restaurant.groupBy({ by: ['onboardingStatus'], _count: true, where: cities ? { cityId: { in: cities } } : {} }),
      p.rider.groupBy({ by: ['onboardingStatus'], _count: true }),
      p.customer.count(),
      p.media.count({ where: { deletedAt: null } }),
      request.auth.admin.global.has('audit.view') ? p.auditLog.findMany({ orderBy: { id: 'desc' }, take: 8 }) : Promise.resolve(null),
    ]);
    return {
      setup: {
