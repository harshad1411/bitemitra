 ...
 * @param {{ audience: 'CUSTOMER' | 'ADMIN', pastOrderCount?: number }} opts
 */
export async function quoteCart(app, request, body, { audience }) {
  ...
  for (const l of body.lines) {
    ...
    valid.push({
      ...
      quantity: l.quantity,
      addons: chosen.map((a) => ({ addonId: a.id, name: a.name, basePricePaise: a.basePricePaise })),
  ...
  const base = {
    ...
    canCheckout: false,
    checkoutNote: 'Checkout and payment arrive in Phase 5.',
  };
  if (!valid.length) return { ...base, lines: [], bill: null, issues };

  // ── Rules, offers, settings → engine ──
  const [rules, coupon, promotions, settings] = await Promise.all([
    loadRuleSet(
      prisma,
      ruleTargets({
        ...ctx,
    ...
    body.couponCode ? prisma.coupon.findUnique({ where: { code: body.couponCode } }) : null,
  ...
  try {
    q = quote(
      { lines: valid },
      {
        ...
        pastOrderCount: 0, // no orders before Phase 5 (D-52)
      },
      rules,
      settings,
      {
        coupon: coupon ? toEngineOffer(coupon, { userUsedCount: 0 }) : null,
  ...
  if (audience === 'ADMIN') return { ...customer, engine: q }; // restaurant, rider, platform and rules — never for customers
  return customer;
}
