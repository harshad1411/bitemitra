...
const serviceability = (lat, lng) =>
  ctx.app.inject({
    method: 'GET',
    url: `/v1/geo/serviceability?lat=${lat}&lng=${lng}`,
...
describe('admin geography', () => {
  it('adds a new city with zones without code changes (multi-city, OD-6)', async () => {
    ...
    const zone = await ctx.app.inject({
      method: 'POST',
      ...
      payload: {
        ...
        slug: 'navrangpura',
        name: 'Navrangpura',
        geometry: square(72.54, 23.03, 72.57, 23.05),
      },
    });
    ...
    const noReason = await ctx.app.inject({
      method: 'PATCH',
      url: `/v1/admin/geo/cities/${cityId}`,
      headers: bearer(token),
    ...
    const launch = await ctx.app.inject({
      ...
      url: `/v1/admin/geo/cities/${cityId}`,
      headers: bearer(token),
      payload: { isActive: true, reason: 'launch' },
    });
    expect(launch.statusCode).toBe(200);
    expect((await serviceability(23.04, 72.555)).json()).toMatchObject({
      serviceable: true,
      city: { slug: 'ahmedabad' },
    });
  ...
  it('rejects invalid polygons and duplicate slugs', async () => {
    ...
    const bad = await ctx.app.inject({
      ...
      url: '/v1/admin/geo/zones',
      headers: bearer(token),
      payload: { cityId: unjha.id, slug: 'open', name: 'Open', geometry: open },
    });
    ...
    const dup = await ctx.app.inject({
      ...
      payload: {
        ...
        slug: 'unjha-central',
        name: 'Dup',
        geometry: square(72.3, 23.7, 72.31, 23.71),
      },
    });
  ...
  it('service areas restrict a zone; deactivating one takes effect immediately', async () => {
    ...
    const area = await ctx.app.inject({
      method: 'POST',
      url: '/v1/admin/geo/service-areas',
      headers: bearer(token),
      payload: {
        zoneId: central.id,
        name: 'Market radius',
    ...
    await ctx.app.inject({
      method: 'PATCH',
      url: `/v1/admin/geo/service-areas/${area.json().id}`,
      headers: bearer(token),
  ...
  it('paginates cities with a cursor', async () => {
    const page1 = (
      await ctx.app.inject({ method: 'GET', url: '/v1/admin/geo/cities?limit=2', headers: bearer(token) })
    ).json();
    ...
    const page2 = (
      await ctx.app.inject({
        method: 'GET',
        url: `/v1/admin/geo/cities?limit=2&cursor=${page1.nextCursor}`,
        headers: bearer(token),
      })
    ).json();
