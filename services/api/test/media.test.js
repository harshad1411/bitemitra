...
const upload = (file, fields = {}, extraHeaders = {}) => {
  ...
  return ctx.app.inject({
    method: 'POST',
    url: '/v1/admin/media',
    headers: { ...bearer(token, 'ADMIN', extraHeaders), 'content-type': body.contentType },
...
describe('media library', () => {
  it('stores an image, records an outbox event and an audit entry, and serves the original', async () => {
    const res = await upload(
      { filename: 'banner.png', contentType: 'image/png', content: TINY_PNG },
      { altText: 'Festival banner', tags: 'banner, diwali' },
    );
    ...
    expect(await ctx.prisma.auditLog.count({ where: { entityId: media.id, action: 'media.upload' } })).toBe(
      1,
    );
  ...
  it('trusts file content, not the claimed type: text, SVG and oversize files are refused', async () => {
    ...
    const big = await upload({
      filename: 'big.png',
      contentType: 'image/png',
      content: Buffer.concat([TINY_PNG, Buffer.alloc(5000)]),
    });
  ...
  it('search, edit alt text, soft delete; deleted files are no longer served', async () => {
    ...
    const found = await ctx.app.inject({
      method: 'GET',
      url: '/v1/admin/media?q=thali',
      headers: bearer(token),
    });
    expect(found.json().items.map((m) => m.id)).toContain(created.id);
    const patched = await ctx.app.inject({
      method: 'PATCH',
      url: `/v1/admin/media/${created.id}`,
      headers: bearer(token),
      payload: { altText: 'A Gujarati thali' },
    });
  ...
  it('file paths cannot escape the storage root', async () => {
    for (const url of [
      '/v1/media/files/../../etc/passwd',
      '/v1/media/files/media/x/original.png',
      '/v1/media/files/%2e%2e%2fsecret',
    ]) {
  ...
  it('upload needs media.manage', async () => {
    ...
    const res = await ctx.app.inject({
      method: 'POST',
      url: '/v1/admin/media',
      headers: { ...bearer(support.accessToken), 'content-type': body.contentType },
      payload: body.payload,
    });
