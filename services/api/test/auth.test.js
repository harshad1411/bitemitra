...
import { DEMO_ACCOUNTS, SUPER, adminLogin, bearer, headers, lastOtp, mobileLogin, startTestApp } from './helpers.js';
...
const otpRequest = (appId, destination, channel = 'SMS', extra = {}) =>
  ctx.app.inject({ method: 'POST', url: '/v1/auth/otp/request', headers: headers(appId, extra), payload: { channel, destination } });
const otpVerify = (appId, challengeId, code) =>
  ctx.app.inject({ method: 'POST', url: '/v1/auth/otp/verify', headers: headers(appId), payload: { challengeId, code } });
...
describe('phone OTP sign-in', () => {
  it('signs a new customer in and creates the customer profile', async () => {
    ...
    expect(body.me).toMatchObject({ appId: 'CUSTOMER', access: { status: 'OK' }, user: { phone: '+919876500001', phoneVerified: true } });
  ...
  it('email OTP works once enabled in settings', async () => {
    ...
    const put = await ctx.app.inject({
      ...
      payload: { key: 'auth.methods', scope: 'GLOBAL', value: { CUSTOMER: ['PHONE_OTP', 'EMAIL_OTP'], RESTAURANT: ['PHONE_OTP'], RIDER: ['PHONE_OTP'], ADMIN: ['PASSWORD'] }, reason: 'enable email OTP for test' },
    });
  ...
  it('social sign-in is honestly unavailable', async () => {
    const res = await ctx.app.inject({ method: 'POST', url: '/v1/auth/social/google', headers: headers('CUSTOMER'), payload: {} });
...
describe('approval is separate from authentication (OD-13)', () => {
  ...
  it('restaurant partner app', async () => {
    ...
    expect((await statusFor('RESTAURANT', DEMO_ACCOUNTS.draftRestaurantManager)).access.status).toBe('PENDING_APPROVAL');
...
describe('sessions', () => {
  it('rotates refresh tokens and revokes the family when an old token is reused', async () => {
    ...
    const refresh = (token) => ctx.app.inject({ method: 'POST', url: '/v1/auth/refresh', headers: headers('CUSTOMER'), payload: { refreshToken: token } });
    ...
    const me = await ctx.app.inject({ method: 'GET', url: '/v1/me', headers: bearer(next.accessToken, 'CUSTOMER') });
    expect(me.statusCode).toBe(401);
  });

  it('tokens are bound to their app', async () => {
    const login = await mobileLogin(ctx, 'CUSTOMER', '9876500021');
    const asRider = await ctx.app.inject({ method: 'GET', url: '/v1/me', headers: bearer(login.accessToken, 'RIDER') });
    expect(asRider.statusCode).toBe(401);
    const refreshAsRider = await ctx.app.inject({ method: 'POST', url: '/v1/auth/refresh', headers: headers('RIDER'), payload: { refreshToken: login.refreshToken } });
  ...
  it('logout revokes the session and expired access tokens report TOKEN_EXPIRED', async () => {
    ...
    ctx.clock.advance(0);
    const out = await ctx.app.inject({ method: 'POST', url: '/v1/auth/logout', headers: bearer(login.accessToken, 'CUSTOMER') });
    expect(out.statusCode).toBe(204);
    expect((await ctx.app.inject({ method: 'GET', url: '/v1/me', headers: bearer(login.accessToken, 'CUSTOMER') })).statusCode).toBe(401);
  });
...
describe('admin sign-in', () => {
  it('signs in, sets an httpOnly SameSite=Strict refresh cookie and refreshes from it', async () => {
    ...
    const refreshed = await ctx.app.inject({ method: 'POST', url: '/v1/auth/refresh', headers: headers('ADMIN'), cookies: { jz_admin_rt: login.cookie.value } });
    expect(refreshed.statusCode).toBe(200);
    expect(refreshed.json().refreshToken).toBeUndefined();
    expect((await ctx.app.inject({ method: 'POST', url: '/v1/auth/refresh', headers: headers('ADMIN') })).statusCode).toBe(401);
  });

  it('uses one generic message for unknown users and wrong passwords, then locks after 5 failures', async () => {
    const attempt = (email, password) => ctx.app.inject({ method: 'POST', url: '/v1/admin/auth/login', headers: headers('ADMIN'), payload: { email, password } });
    ...
    const audits = await ctx.prisma.auditLog.findMany({ where: { action: { in: ['admin.login_failed', 'admin.locked'] } } });
